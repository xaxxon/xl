...
#define nsel_REQUIRES_T(...) \
    typename = typename std::enable_if< (__VA_ARGS__), xl::expected_detail::enabler >::type
...
public:
    using value_type = T;
    using error_type = E;
    using unexpected_type = xl::unexpected_type<E>;
...
//        std::is_copy_constructible<T>::value )
    ...
    nsel_constexpr14 explicit expected( U && v, nsel_REQUIRES(
        ...
        && !std::is_same<xl::unexpected_type<E>, typename std20::remove_cvref<U>::type>::value
    ...
    nsel_constexpr14 explicit expected( xl::unexpected_type<G> const & error, nsel_REQUIRES(
    ...
    template< typename G = E >
    nsel_constexpr14 /*non-explicit*/ expected( xl::unexpected_type<G> const & error, nsel_REQUIRES(
    ...
    template< typename G = E >
    nsel_constexpr14 explicit expected( xl::unexpected_type<G> && error, nsel_REQUIRES(
    ...
    template< typename G = E >
    nsel_constexpr14 /*non-explicit*/ expected( xl::unexpected_type<G> && error, nsel_REQUIRES(
...
template< typename T, typename E >
struct hash< xl::expected<T,E> >
{
    using result_type = typename hash<T>::result_type;
    using argument_type = xl::expected<T,E>;
...
template< typename E >
struct hash< xl::expected<void,E> >
{
};
